...
{
	public partial class frmMecCCAdmDifParDiv : Form
	{
		frmMenu frmMenu;
		public frmMecCCAdmDifParDiv(frmMenu form)
		{
			this.frmMenu = form;
		...
		void Consultar()
		{
			try
			{
				...
				else
				{
					...
					dgvConsulta.DataSource = MecCCAdmDifParDiv();
		...
		public List<MecCCAdmDifParDiv> MecCCAdmDifParDiv() // Método que retorna a conferência dos erros 
		{

			string qyMec = "SELECT PIMSCS.APT_MEC_HE.DT_OPERACAO DATA,        PIMSCS.APT_MEC_HE.NO_BOLETIM  BOLETIM,        PIMSCS.APT_MEC_HE.CD_EQUIPTO  COD_EQUIPAMENTO,        PIMSCS.APT_MEC_DE.CD_SAFRA    SAFRA,        PIMSCS.APT_MEC_HE.CD_FUNC     COD_FUNC,        PIMSCS.FUNCIONARS.DE_FUNC     FUNCIONARIO,        PIMSCS.APT_MEC_DE.CD_CCUSTO   COD_CCUSTO,        PIMSCS.CCUSTOS.DE_CCUSTO      CCUSTO,        PIMSCS.APT_MEC_DE.CD_OPERACAO COD_OPERACAO,        PIMSCS.OPERACOES.DE_OPERACAO  OPERACAO,        PIMSCS.APT_MEC_DE.CD_UPNIVEL1 COD_PARCERIA,        PIMSCS.UPNIVEL1.DA_UPNIVEL1   PARCERIA,        PIMSCS.APT_MEC_DE.CD_UPNIVEL3 TALHAO,        PIMSCS.APT_MEC_HE.CD_USR_DML  USUARIO   FROM PIMSCS.APT_MEC_DE,        PIMSCS.APT_MEC_HE,        PIMSCS.CCUSTOS,        PIMSCS.OPERACOES,        PIMSCS.UPNIVEL1,        PIMSCS.FUNCIONARS  WHERE PIMSCS.APT_MEC_DE.FG_TP_APTO = PIMSCS.APT_MEC_HE.FG_TP_APTO    AND PIMSCS.APT_MEC_DE.NO_BOLETIM = PIMSCS.APT_MEC_HE.NO_BOLETIM    AND PIMSCS.APT_MEC_DE.CD_CCUSTO = PIMSCS.CCUSTOS.CD_CCUSTO    AND PIMSCS.APT_MEC_DE.CD_UPNIVEL1 = PIMSCS.UPNIVEL1.CD_UPNIVEL1    AND PIMSCS.APT_MEC_DE.CD_OPERACAO = PIMSCS.OPERACOES.CD_OPERACAO    AND PIMSCS.APT_MEC_DE.CD_CCUSTO = PIMSCS.CCUSTOS.CD_CCUSTO    AND PIMSCS.APT_MEC_HE.CD_FUNC = PIMSCS.FUNCIONARS.CD_FUNC    AND PIMSCS.APT_MEC_HE.DT_OPERACAO >= :DataIni    AND PIMSCS.APT_MEC_HE.DT_OPERACAO <= :DataFim    AND PIMSCS.APT_MEC_DE.CD_CCUSTO >= 100000    AND PIMSCS.APT_MEC_DE.CD_CCUSTO <= 999999    AND PIMSCS.APT_MEC_DE.CD_UPNIVEL1 <> '99999'    AND PIMSCS.APT_MEC_DE.CD_CCUSTO NOT IN        (190000,         190101,         190121,         190102,         190122,         190103,         190123,         190104,         190105,         190106,         190107,         190108,         190109,         190202,         190203,         190112,         190113,         160900,         261100,         160600,         900000) UNION ALL SELECT PIMSCS.APT_MEC_HE.DT_OPERACAO DATA,        PIMSCS.APT_MEC_HE.NO_BOLETIM  BOLETIM,        PIMSCS.APT_MEC_HE.CD_EQUIPTO  COD_EQUIPAMENTO,        PIMSCS.APT_MEC_DE.CD_SAFRA    SAFRA,        PIMSCS.APT_MEC_HE.CD_FUNC     COD_FUNC,        PIMSCS.FUNCIONARS.DE_FUNC     FUNCIONARIO,        PIMSCS.APT_MEC_DE.CD_CCUSTO   COD_CCUSTO,        PIMSCS.CCUSTOS.DE_CCUSTO      CCUSTO,        PIMSCS.APT_MEC_DE.CD_OPERACAO COD_OPERACAO,        PIMSCS.OPERACOES.DE_OPERACAO  OPERACAO,        PIMSCS.APT_MEC_DE.CD_UPNIVEL1 COD_PARCERIA,        PIMSCS.UPNIVEL1.DA_UPNIVEL1   PARCERIA,        PIMSCS.APT_MEC_DE.CD_UPNIVEL3 TALHAO,        PIMSCS.APT_MEC_HE.CD_USR_DML  USUARIO   FROM PIMSCS.APT_MEC_DE,        PIMSCS.APT_MEC_HE,        PIMSCS.CCUSTOS,        PIMSCS.OPERACOES,        PIMSCS.UPNIVEL1,        PIMSCS.FUNCIONARS  WHERE PIMSCS.APT_MEC_DE.FG_TP_APTO = PIMSCS.APT_MEC_HE.FG_TP_APTO    AND PIMSCS.APT_MEC_DE.NO_BOLETIM = PIMSCS.APT_MEC_HE.NO_BOLETIM    AND PIMSCS.APT_MEC_DE.CD_CCUSTO = PIMSCS.CCUSTOS.CD_CCUSTO    AND PIMSCS.APT_MEC_DE.CD_UPNIVEL1 = PIMSCS.UPNIVEL1.CD_UPNIVEL1    AND PIMSCS.APT_MEC_DE.CD_OPERACAO = PIMSCS.OPERACOES.CD_OPERACAO    AND PIMSCS.APT_MEC_DE.CD_CCUSTO = PIMSCS.CCUSTOS.CD_CCUSTO    AND PIMSCS.APT_MEC_HE.CD_FUNC = PIMSCS.FUNCIONARS.CD_FUNC    AND PIMSCS.APT_MEC_HE.DT_OPERACAO >= :DataIni    AND PIMSCS.APT_MEC_HE.DT_OPERACAO <= :DataFim    AND PIMSCS.APT_MEC_DE.CD_CCUSTO = 160900    AND PIMSCS.APT_MEC_DE.CD_UPNIVEL1 <> '90003' UNION ALL SELECT PIMSCS.APT_MEC_HE.DT_OPERACAO DATA,        PIMSCS.APT_MEC_HE.NO_BOLETIM  BOLETIM,        PIMSCS.APT_MEC_HE.CD_EQUIPTO  COD_EQUIPAMENTO,        PIMSCS.APT_MEC_DE.CD_SAFRA    SAFRA,        PIMSCS.APT_MEC_HE.CD_FUNC     COD_FUNC,        PIMSCS.FUNCIONARS.DE_FUNC     FUNCIONARIO,        PIMSCS.APT_MEC_DE.CD_CCUSTO   COD_CCUSTO,        PIMSCS.CCUSTOS.DE_CCUSTO      CCUSTO,        PIMSCS.APT_MEC_DE.CD_OPERACAO COD_OPERACAO,        PIMSCS.OPERACOES.DE_OPERACAO  OPERACAO,        PIMSCS.APT_MEC_DE.CD_UPNIVEL1 COD_PARCERIA,        PIMSCS.UPNIVEL1.DA_UPNIVEL1   PARCERIA,        PIMSCS.APT_MEC_DE.CD_UPNIVEL3 TALHAO,        PIMSCS.APT_MEC_HE.CD_USR_DML  USUARIO   FROM PIMSCS.APT_MEC_DE,        PIMSCS.APT_MEC_HE,        PIMSCS.CCUSTOS,        PIMSCS.OPERACOES,        PIMSCS.UPNIVEL1,        PIMSCS.FUNCIONARS  WHERE PIMSCS.APT_MEC_DE.FG_TP_APTO = PIMSCS.APT_MEC_HE.FG_TP_APTO    AND PIMSCS.APT_MEC_DE.NO_BOLETIM = PIMSCS.APT_MEC_HE.NO_BOLETIM    AND PIMSCS.APT_MEC_DE.CD_CCUSTO = PIMSCS.CCUSTOS.CD_CCUSTO    AND PIMSCS.APT_MEC_DE.CD_UPNIVEL1 = PIMSCS.UPNIVEL1.CD_UPNIVEL1    AND PIMSCS.APT_MEC_DE.CD_OPERACAO = PIMSCS.OPERACOES.CD_OPERACAO    AND PIMSCS.APT_MEC_DE.CD_CCUSTO = PIMSCS.CCUSTOS.CD_CCUSTO    AND PIMSCS.APT_MEC_HE.CD_FUNC = PIMSCS.FUNCIONARS.CD_FUNC    AND PIMSCS.APT_MEC_HE.DT_OPERACAO >= :DataIni    AND PIMSCS.APT_MEC_HE.DT_OPERACAO <= :DataFim    AND PIMSCS.APT_MEC_DE.CD_CCUSTO = 261100    AND PIMSCS.APT_MEC_DE.CD_UPNIVEL1 NOT IN ('90001', '90002')    UNION ALL SELECT PIMSCS.APT_MEC_HE.DT_OPERACAO DATA,        PIMSCS.APT_MEC_HE.NO_BOLETIM  BOLETIM,        PIMSCS.APT_MEC_HE.CD_EQUIPTO  COD_EQUIPAMENTO,        PIMSCS.APT_MEC_DE.CD_SAFRA    SAFRA,        PIMSCS.APT_MEC_HE.CD_FUNC     COD_FUNC,        PIMSCS.FUNCIONARS.DE_FUNC     FUNCIONARIO,        PIMSCS.APT_MEC_DE.CD_CCUSTO   COD_CCUSTO,        PIMSCS.CCUSTOS.DE_CCUSTO      CCUSTO,        PIMSCS.APT_MEC_DE.CD_OPERACAO COD_OPERACAO,        PIMSCS.OPERACOES.DE_OPERACAO  OPERACAO,        PIMSCS.APT_MEC_DE.CD_UPNIVEL1 COD_PARCERIA,        PIMSCS.UPNIVEL1.DA_UPNIVEL1   PARCERIA,        PIMSCS.APT_MEC_DE.CD_UPNIVEL3 TALHAO,        PIMSCS.APT_MEC_HE.CD_USR_DML  USUARIO   FROM PIMSCS.APT_MEC_DE,        PIMSCS.APT_MEC_HE,        PIMSCS.CCUSTOS,        PIMSCS.OPERACOES,        PIMSCS.UPNIVEL1,        PIMSCS.FUNCIONARS  WHERE PIMSCS.APT_MEC_DE.FG_TP_APTO = PIMSCS.APT_MEC_HE.FG_TP_APTO    AND PIMSCS.APT_MEC_DE.NO_BOLETIM = PIMSCS.APT_MEC_HE.NO_BOLETIM    AND PIMSCS.APT_MEC_DE.CD_CCUSTO = PIMSCS.CCUSTOS.CD_CCUSTO    AND PIMSCS.APT_MEC_DE.CD_UPNIVEL1 = PIMSCS.UPNIVEL1.CD_UPNIVEL1    AND PIMSCS.APT_MEC_DE.CD_OPERACAO = PIMSCS.OPERACOES.CD_OPERACAO    AND PIMSCS.APT_MEC_DE.CD_CCUSTO = PIMSCS.CCUSTOS.CD_CCUSTO    AND PIMSCS.APT_MEC_HE.CD_FUNC = PIMSCS.FUNCIONARS.CD_FUNC    AND PIMSCS.APT_MEC_HE.DT_OPERACAO >= :DataIni    AND PIMSCS.APT_MEC_HE.DT_OPERACAO <= :DataFim    AND PIMSCS.APT_MEC_DE.CD_CCUSTO = 160600    AND PIMSCS.APT_MEC_DE.CD_UPNIVEL1 <> '90600'  ORDER BY DATA, BOLETIM, COD_CCUSTO";
			...
			{
				var erroMec = conn.Query<MecCCAdmDifParDiv>(qyMec, param).ToList();
