...
{
	public partial class frmMecHoraOp : Form
	{
		frmMenu frmMenu;
		public frmMecHoraOp(frmMenu form)
		{
			this.frmMenu = form;
		...
		void Consultar()
		{
			try
			{
				...
				else
				{
					...
					dgvConsulta.DataSource = MecHoraOp();
		...
		public List<MecHoraOp> MecHoraOp() // Método que retorna a conferência dos erros 
		{

			string qyMec = "SELECT A.DT_OPERACAO DATA,        A.NO_BOLETIM  BOLETIM,        B.CD_SAFRA    SAFRA,        A.CD_FUNC     COD_FUNC,        C.DE_FUNC     FUNCIONARIO,        H.CD_EQUIPTO  COD_EQUIPAMENTO,        D.DE_MODELO   EQUIPAMENTO,        A.QT_TOT_HORAS HORAS_OPERADOR,        B.CD_EQ_IMPL1 IMPLEMENTO,        B.CD_CCUSTO   COD_CCUSTO,        B.CD_OPERACAO COD_OPERACAO,        B.CD_UPNIVEL1 COD_PARCERIA,        B.CD_UPNIVEL3 TALHAO,        B.CD_USR_DML  USUARIO   FROM PIMSCS.APT_MEC_HE A,        PIMSCS.APT_MEC_DE B,        PIMSCS.FUNCIONARS C,        PIMSCS.MODELOS    D,        PIMSCS.CCUSTOS    E,        PIMSCS.OPERACOES  F,        PIMSCS.UPNIVEL1   G,        PIMSCS.EQUIPTOS   H  WHERE A.NO_BOLETIM = B.NO_BOLETIM    AND A.CD_FUNC = C.CD_FUNC    AND A.CD_EQUIPTO = H.CD_EQUIPTO    AND H.CD_MODELO = D.CD_MODELO    AND B.CD_CCUSTO = E.CD_CCUSTO    AND B.CD_OPERACAO = F.CD_OPERACAO    AND B.CD_UPNIVEL1 = G.CD_UPNIVEL1    AND A.QT_TOT_HORAS > 24    AND A.DT_OPERACAO BETWEEN :DataIni AND :DataFim  ORDER BY A.DT_OPERACAO,           A.NO_BOLETIM,           H.CD_EQUIPTO,           A.QT_TOT_HORAS,           B.CD_EQ_IMPL1,           B.CD_CCUSTO,           B.CD_OPERACAO,           B.CD_UPNIVEL1,           B.CD_UPNIVEL3";
			...
			{
				var erroMec = conn.Query<MecHoraOp>(qyMec, param).ToList();
