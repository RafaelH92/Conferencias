...
{
	public partial class frmManuParDiversas : Form
	{
		frmMenu frmMenu;
		...
		void Consultar()
		{
			try
			{
				...
				else
				{
					...
					dgvConsulta.DataSource = ManuParDiversas();
		...
		public List<ManuParDiversas> ManuParDiversas() // Método que retorna a conferência dos erros 
		{

			string qyManu = "SELECT A.NO_BOLETIM         BOLETIM,        A.DT_OPERACAO        DATA,        A.CD_FUNC            COD_FUNC,        G.DE_FUNC            FUNCIONARIO,        A.CD_SAFRA           SAFRA,        A.CD_CCUSTO          COD_CCUSTO,        CCUSTOS.DE_CCUSTO    CCUSTOS,        A.CD_UPNIVEL1        AS COD_PARCERIA,        UPNIVEL1.DA_UPNIVEL1 PARCERIA,        A.CD_UPNIVEL3        TALHAO,               A.CD_OPERACAO COD_OPERACAO,        B.DE_OPERACAO OPERACAO    FROM PIMSCS.APT_MANU   A,        PIMSCS.OPERACOES  B,        PIMSCS.CCUSTOS,        PIMSCS.UPNIVEL1,        PIMSCS.FUNCIONARS G   WHERE A.DT_OPERACAO >= :DataIni    and A.DT_OPERACAO <= :DataFim   AND A.CD_CCUSTO >= 190000    AND A.CD_CCUSTO <= 199999    AND A.CD_UPNIVEL1 = '99999'    AND A.CD_FUNC = G.CD_FUNC    AND A.CD_OPERACAO = B.CD_OPERACAO    AND CCUSTOS.CD_CCUSTO = A.CD_CCUSTO    AND A.CD_UPNIVEL1 = UPNIVEL1.CD_UPNIVEL1   ORDER BY A.DT_OPERACAO, A.NO_BOLETIM";
			...
			{
				var erroManu = conn.Query<ManuParDiversas>(qyManu, param).ToList();
