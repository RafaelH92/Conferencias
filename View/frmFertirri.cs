...
namespace CONFERENCIAS
{
	public partial class frmFertirri : Form
	{
		...
		void Consultar()
		{
			try
			{
				...
				else
				{
					...
					else if (rbTalhao.Checked)
					{
						if (txtFornecedor.Text == string.Empty && txtOperacao.Text == string.Empty)
						{
							dgvConsulta.DataSource = fertirriTlAll();
						}
						else if (txtFornecedor.Text != string.Empty && txtOperacao.Text == string.Empty)
						{
							dgvConsulta.DataSource = fertirriTlFor();
						}
						else if (txtFornecedor.Text == string.Empty && txtOperacao.Text != string.Empty)
						{
							dgvConsulta.DataSource = fertirriTlOp();
						}
						else if (txtFornecedor.Text != string.Empty && txtOperacao.Text != string.Empty)
						{
							dgvConsulta.DataSource = fertirriTlForOp();
						}
					}
					else if (rbParceria.Checked)
					{
						if (txtFornecedor.Text == string.Empty && txtOperacao.Text == string.Empty)
						{
							dgvConsulta.DataSource = fertirriParAll();
						}
						else if (txtFornecedor.Text != string.Empty && txtOperacao.Text == string.Empty)
						{
							dgvConsulta.DataSource = fertirriParFor();
						}
						else if (txtFornecedor.Text == string.Empty && txtOperacao.Text != string.Empty)
						{
							dgvConsulta.DataSource = fertirriParOp();
						}
						else if (txtFornecedor.Text != string.Empty && txtOperacao.Text != string.Empty)
						{
							dgvConsulta.DataSource = fertirriParForOp();
						}
		...
		public List<FertirriTalhao> fertirriTlAll() // Método que retorna a os dados de brocas
		{
			...
			using (IDbConnection conn = new OracleConnection("Password=pims;User ID=CONSULTOR;Data Source=ORA81_TCP"))
			{
				var fertirriTl = conn.Query<FertirriTalhao>(qyFertirriTl, param).ToList();

				return fertirriTl;
			}
		}

		public List<FertirriTalhao> fertirriTlOp() // Método que retorna a os dados de brocas
		{
		...
		public List<FertirriTalhao> fertirriTlForOp() // Método que retorna a os dados de brocas
		{
		...
		public List<FertirriTalhao> fertirriTlFor() // Método que retorna a os dados de brocas
		{
		...
		public List<FertirriParc> fertirriParAll() // Método que retorna a os dados de brocas
		{

			string qyFertirriPar = " SELECT A.PARCERIA,        A.OPERACAO,        A.AREA,        B.HORAS,        C.M3,        D.K2O,        ROUND(B.HORAS / A.AREA, 2) AS HR_HA,        ROUND(C.M3 / B.HORAS, 2) AS M3_HR,        ROUND(C.M3 / A.AREA, 2) AS M3_HA,        ROUND(D.K2O / A.AREA, 2) AS K2O_HA,        ROUND(D.K2O / C.M3, 2) AS K2O_M3    FROM (                 SELECT H.CD_UPNIVEL1 AS COD,                 G.DE_UPNIVEL1 AS PARCERIA,                 O.DE_OPERACAO AS OPERACAO,                 SUM(H.QT_PROD_REC) AS AREA                   FROM PIMSCS.HISTMANEJO H, PIMSCS.UPNIVEL1 G, PIMSCS.OPERACOES O          WHERE H.CD_UPNIVEL1 = G.CD_UPNIVEL1            AND H.CD_OPERACAO = O.CD_OPERACAO            AND H.DT_HISTORICO BETWEEN :DataIni AND :DataFim            AND H.FG_ORIGEM = 'A'            AND H.CD_OPERACAO IN (73, 176)                  GROUP BY H.CD_UPNIVEL1, G.DE_UPNIVEL1, O.DE_OPERACAO          ORDER BY H.CD_UPNIVEL1, G.DE_UPNIVEL1, O.DE_OPERACAO) A,               (SELECT H.CD_UPNIVEL1 AS COD,                G.DE_UPNIVEL1 AS PARCERIA,                O.DE_OPERACAO AS OPERACAO,                SUM(H.QT_PROD_REC) AS HORAS           FROM PIMSCS.HISTMANEJO H, PIMSCS.UPNIVEL1 G, PIMSCS.OPERACOES O          WHERE H.CD_UPNIVEL1 = G.CD_UPNIVEL1            AND H.CD_OPERACAO = O.CD_OPERACAO            AND H.DT_HISTORICO BETWEEN :DataIni AND :DataFim            AND H.FG_ORIGEM = 'V'            AND H.CD_UNID_MED = 'HD'            AND H.CD_OPERACAO IN (73, 176)          GROUP BY H.CD_UPNIVEL1, G.DE_UPNIVEL1, O.DE_OPERACAO, O.DE_OPERACAO          ORDER BY H.CD_UPNIVEL1, G.DE_UPNIVEL1, O.DE_OPERACAO) B,               (SELECT H.CD_UPNIVEL1 AS COD,                G.DE_UPNIVEL1 AS PARCERIA,                O.DE_OPERACAO AS OPERACAO,                SUM(H.QT_PROD_REC) AS M3           FROM PIMSCS.HISTMANEJO H, PIMSCS.UPNIVEL1 G, PIMSCS.OPERACOES O          WHERE H.CD_UPNIVEL1 = G.CD_UPNIVEL1            AND H.CD_OPERACAO = O.CD_OPERACAO            AND H.DT_HISTORICO BETWEEN :DataIni AND :DataFim            AND H.FG_ORIGEM = 'V'            AND H.CD_UNID_MED = 'M³'            AND H.CD_HIST = 'R'          GROUP BY H.CD_UPNIVEL1, G.DE_UPNIVEL1, O.DE_OPERACAO          ORDER BY H.CD_UPNIVEL1, G.DE_UPNIVEL1, O.DE_OPERACAO) C,               (SELECT H.CD_UPNIVEL1 AS COD,                G.DE_UPNIVEL1 AS PARCERIA,                O.DE_OPERACAO AS OPERACAO,                SUM(H.QT_PROD_REC) AS K2O           FROM PIMSCS.HISTMANEJO H, PIMSCS.UPNIVEL1 G, PIMSCS.OPERACOES O          WHERE H.CD_UPNIVEL1 = G.CD_UPNIVEL1            AND H.CD_OPERACAO = O.CD_OPERACAO            AND H.DT_HISTORICO BETWEEN :DataIni AND :DataFim            AND H.FG_ORIGEM = 'V'            AND H.CD_RECURSO = 9008            AND H.CD_HIST = 'R'          GROUP BY H.CD_UPNIVEL1, G.DE_UPNIVEL1, O.DE_OPERACAO          ORDER BY H.CD_UPNIVEL1, G.DE_UPNIVEL1, O.DE_OPERACAO) D   WHERE A.COD = B.COD    AND A.COD = C.COD    AND A.COD = D.COD    AND A.PARCERIA = B.PARCERIA    AND A.PARCERIA = C.PARCERIA    AND A.PARCERIA = D.PARCERIA    AND A.OPERACAO = B.OPERACAO    AND A.OPERACAO = C.OPERACAO    AND A.OPERACAO = D.OPERACAO       UNION ALL SELECT A.PARCERIA,        A.OPERACAO,        A.AREA,        B.HORAS,        C.M3,        D.K2O,        ROUND(B.HORAS / A.AREA, 2) AS HR_HA,        ROUND(C.M3 / B.HORAS, 2) AS M3_HR,        ROUND(C.M3 / A.AREA, 2) AS M3_HA,        ROUND(D.K2O / A.AREA, 2) AS K2O_HA,        ROUND(D.K2O / C.M3, 2) AS K2O_M3    FROM (                 SELECT CASE                   WHEN G.DE_UPNIVEL1 IS NULL THEN                    'NULO'                   ELSE                    'TOTAL_GERAL'                 END AS PARCERIA,                 O.DE_OPERACAO AS OPERACAO,                 SUM(H.QT_PROD_REC) AS AREA                   FROM PIMSCS.HISTMANEJO H, PIMSCS.UPNIVEL1 G, PIMSCS.OPERACOES O          WHERE H.CD_UPNIVEL1 = G.CD_UPNIVEL1            AND H.CD_OPERACAO = O.CD_OPERACAO            AND H.DT_HISTORICO BETWEEN :DataIni AND :DataFim            AND H.FG_ORIGEM = 'A'            AND H.CD_OPERACAO IN (73, 176)                  GROUP BY (CASE                      WHEN G.DE_UPNIVEL1 IS NULL THEN                       'NULO'                      ELSE                       'TOTAL_GERAL'                    END),                    O.DE_OPERACAO          ) A,               (SELECT CASE                  WHEN G.DE_UPNIVEL1 IS NULL THEN                   'NULO'                  ELSE                   'TOTAL_GERAL'                END AS PARCERIA,                O.DE_OPERACAO AS OPERACAO,                SUM(H.QT_PROD_REC) AS HORAS           FROM PIMSCS.HISTMANEJO H, PIMSCS.UPNIVEL1 G, PIMSCS.OPERACOES O          WHERE H.CD_UPNIVEL1 = G.CD_UPNIVEL1            AND H.CD_OPERACAO = O.CD_OPERACAO            AND H.DT_HISTORICO BETWEEN :DataIni AND :DataFim            AND H.FG_ORIGEM = 'V'            AND H.CD_UNID_MED = 'HD'            AND H.CD_OPERACAO IN (73, 176)          GROUP BY (CASE                     WHEN G.DE_UPNIVEL1 IS NULL THEN                      'NULO'                     ELSE                      'TOTAL_GERAL'                   END),                   O.DE_OPERACAO) B,               (SELECT CASE                  WHEN G.DE_UPNIVEL1 IS NULL THEN                   'NULO'                  ELSE                   'TOTAL_GERAL'                END AS PARCERIA,                O.DE_OPERACAO AS OPERACAO,                SUM(H.QT_PROD_REC) AS M3           FROM PIMSCS.HISTMANEJO H, PIMSCS.UPNIVEL1 G, PIMSCS.OPERACOES O          WHERE H.CD_UPNIVEL1 = G.CD_UPNIVEL1            AND H.CD_OPERACAO = O.CD_OPERACAO            AND H.DT_HISTORICO BETWEEN :DataIni AND :DataFim            AND H.FG_ORIGEM = 'V'            AND H.CD_UNID_MED = 'M³'            AND H.CD_HIST = 'R'          GROUP BY (CASE                     WHEN G.DE_UPNIVEL1 IS NULL THEN                      'NULO'                     ELSE                      'TOTAL_GERAL'                   END),                   O.DE_OPERACAO) C,               (SELECT CASE                  WHEN G.DE_UPNIVEL1 IS NULL THEN                   'NULO'                  ELSE                   'TOTAL_GERAL'                END AS PARCERIA,                O.DE_OPERACAO AS OPERACAO,                SUM(H.QT_PROD_REC) AS K2O           FROM PIMSCS.HISTMANEJO H, PIMSCS.UPNIVEL1 G, PIMSCS.OPERACOES O          WHERE H.CD_UPNIVEL1 = G.CD_UPNIVEL1            AND H.CD_OPERACAO = O.CD_OPERACAO            AND H.DT_HISTORICO BETWEEN :DataIni AND :DataFim            AND H.FG_ORIGEM = 'V'            AND H.CD_RECURSO = 9008            AND H.CD_HIST = 'R'          GROUP BY (CASE                     WHEN G.DE_UPNIVEL1 IS NULL THEN                      'NULO'                     ELSE                      'TOTAL_GERAL'                   END),                   O.DE_OPERACAO) D   WHERE A.PARCERIA = B.PARCERIA    AND A.PARCERIA = C.PARCERIA    AND A.PARCERIA = D.PARCERIA    AND A.OPERACAO = B.OPERACAO    AND A.OPERACAO = C.OPERACAO    AND A.OPERACAO = D.OPERACAO ";
			...
			using (IDbConnection conn = new OracleConnection("Password=pims;User ID=CONSULTOR;Data Source=ORA81_TCP"))
			{
				var fertirriPar = conn.Query<FertirriParc>(qyFertirriPar, param).ToList();

				return fertirriPar;
			}
		}

		public List<FertirriParc> fertirriParOp() // Método que retorna a os dados de brocas
		{
		...
		public List<FertirriParc> fertirriParFor() // Método que retorna a os dados de brocas
		{
		...
		public List<FertirriParc> fertirriParForOp() // Método que retorna a os dados de brocas
		{
