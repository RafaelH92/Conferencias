...
{
	public partial class frmAbastTan : Form
	{
		frmMenu frmMenu;
		...
		void Consultar()
		{
			try
			{
				...
				else
				{
					...
					dgvConsulta.DataSource = abasTan();
		...
		public List<AbasTan> abasTan() // Método que retorna a conferência dos erros 
		{

			string qyAbast = "  SELECT DISTINCT A.NO_BOLETIM AS BOLETIM,        A.DT_OPERACAO AS DATA,        TO_CHAR((A.CD_EQUIPTO || ' - ' || C.DE_MODELO)) AS EQUIPAMENTO,        TO_CHAR((A.CD_MATERIAL || ' - ' || D.DE_MATERIAL)) AS MATERIAL,        E.CP_TANQUE AS TANQUE,        A.QT_ABASTEC AS QUANTIDADE,        A.QT_ABASTEC - E.CP_TANQUE AS DIFERENÇA    FROM PIMSCS.APT_ABAST_DE A,        PIMSCS.EQUIPTOS     B,        PIMSCS.MODELOS      C,        PIMSCS.MATERIAIS    D,        PIMSCS.CONS_MEDIO   E   WHERE A.CD_EQUIPTO = B.CD_EQUIPTO    AND B.CD_MODELO = C.CD_MODELO    AND A.CD_MATERIAL = D.CD_MATERIAL    AND A.CD_EQUIPTO = E.CD_EQUIPTO    AND C.CD_MODELO = E.CD_MODELO    AND A.QT_ABASTEC > E.CP_TANQUE    AND A.DT_OPERACAO BETWEEN :DataIni AND :DataFim   ORDER BY A.DT_OPERACAO, A.NO_BOLETIM, EQUIPAMENTO ";
			...
			{
				var erroAbast = conn.Query<AbasTan>(qyAbast, param).ToList();
